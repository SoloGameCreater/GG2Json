...
set credentialsFile=client_secret_942388970445-ck7iefag4jn02bu92nq94t0o2m1a2ffr.apps.googleusercontent.com.json
...
for /l %%i in (!idxBegin!,1,!idxEnd!) do (
    if %%i LSS %sheetCount% (
        ...
        set cmd=python google_sheets_to_json_batch_oauth.py --sheet_id "!sheets[%%i].id!" --output "output/!sheets[%%i].output!" --format "!sheets[%%i].format!" --credentials "%credentialsFile%"
...
:showHelp
echo ===============================================
echo Google表格数据导出工具 (OAuth 2.0版本)
echo ===============================================
echo 用法: %~nx0 [选项]
echo 选项:
echo   0         - 导出所有表格
